...
#' @author Andreas Handel, edited by Hannah Diehl
#' @export


#### ADAPTED FROM DSAIRM

generate_documentation <- function(docfilename, is_walkthrough = FALSE)
{
    #take HTML file and split it into components for each tab
    tablist = NULL
    tabtitles = c('Overview','Lineup Generation','Examples')
    if (is_walkthrough) {
      tabtitles = c('Data Upload + Lineup Voting', 'Team Voting')
    }
