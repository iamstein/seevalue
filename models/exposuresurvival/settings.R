...
############################################################
app_settings = list()
app_settings$apptitle = "exposuresurvival" # Title of app, to be displayed on top of analyze tab
...
show_subgroup_survival_response_lineup <- function(lineup_data, input) {
  grouped_fits <- survminer::surv_fit(formula = Surv(time, status) ~ quantile,
  ...
  # loop over all datasets and fit a survival model to each dataset
  ...
  #my_lineup <- gridExtra::grid.arrange(grobs = plots,
  ...
  #return(my_lineup)
	...
	#km_fit <- survfit(Surv(time, status) ~ quantile, data = lineup_data, conf.int = 0.95)
...
app_settings$toRegister = data.frame(	
	"longname" = c("Exposure", "Status", "Time"),  # TODO 2: Create a list of display names of the quanity to register to columns.
...
## Preload
app_settings$preload_file = list("datapath"="vignette_data/fake_exp_surv_data.csv")
app_settings$toRegister$preload_columns = c("exposure", "status", "time") 
